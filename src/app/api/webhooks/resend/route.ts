...
const NOT_INTERESTED = ['not interested', 'no thanks', 'remove me', 'stop', 'unsubscribe']
const INTERESTED = ['interested', 'tell me more', 'book', 'demo', 'yes']

function normalizeEmail(raw: unknown) {
  return String(raw || '').trim().toLowerCase().replace(/^mailto:/, '')
}
...
async function findLeadId(admin: ReturnType<typeof createAdminClient>, payload: Record<string, unknown>) {
  ...
  if (emailId) {
    ...
    const match = (data || []).find((r: any) => String(r?.payload?.email_id || '') === emailId)
    if (match?.lead_id) return match.lead_id as string
  }
...
export async function POST(req: Request) {
  ...
  const supplied = req.headers.get('authorization')?.replace(/^Bearer\s+/i, '').trim() || req.headers.get('x-webhook-secret')?.trim() || ''
  ...
  const replyText = String(payload.text || payload.reply_text || payload.body || '')

  if (eventType.includes('reply') || replyText) {
    const status = classifyReply(replyText)
    const { data: lead } = await admin.from('leads').select('notes').eq('id', leadId).maybeSingle()
    const notes = parseNotes((lead?.notes as string | null) || null) as Record<string, unknown>

    const merged = {
      ...notes,
      outcome_label: status === 'interested' ? 'Interested reply captured' : status === 'not_interested' ? 'Not interested reply captured' : 'Reply captured',
    ...
    await admin
      .from('leads')
      .update({ status, notes: JSON.stringify(merged), updated_at: new Date().toISOString() })
      .eq('id', leadId)
