...
function scoreLead(input: z.infer<typeof leadSchema>) {
  ...
  if (['aesthetics', 'trades', 'estate'].some((n) => input.niche.toLowerCase().includes(n))) score += 20
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const payload = rows.map((r) => ({ ...r, contact_email: r.contact_email || null, score: scoreLead(r) }))
