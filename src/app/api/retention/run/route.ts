...
import { renderRetentionMessage, type RetentionBehavior } from '@/lib/retention'
...
export async function POST(req: Request) {
  ...
  const { data: rows, error } = await admin
    ...
    .select('id,behavior,retention_client_id,retention_clients(id,client_name,email,phone,service,clinic_name,objection)')
    .eq('status', 'queued')
    .lte('due_at', now)
  ...
  for (const row of rows || []) {
    const client = Array.isArray(row.retention_clients) ? row.retention_clients[0] : row.retention_clients
    if (!client) {
      await admin.from('retention_tasks').update({ status: 'failed', error: 'client_not_found' }).eq('id', row.id)
    ...
    // idempotency / anti-spam guard: skip if same behavior was sent in last 24h for this client
    ...
    if (client.email) {
      if (await isSuppressed(client.email)) {
        skipped.push({ id: row.id, reason: 'suppressed' })
      } else {
        ...
        else skipped.push({ id: row.id, reason: result.error })
      }
    } else {
      skipped.push({ id: row.id, reason: 'email_missing' })
    }
    ...
    await admin
      ...
      .update({
        ...
        error: delivered ? null : skipped.at(-1)?.reason || 'unknown',
      })
      .eq('id', row.id)
  ...
  return NextResponse.json({ ok: true, sentCount: sent.length, skippedCount: skipped.length, sent, skipped })
}
