export type SitePlatform = 'wordpress' | 'shopify' | 'squarespace' | 'wix' | 'custom'
...
export function buildWidgetSnippet(appUrl: string, token: string) {
  ...
  return `<script defer src="${base}/api/widget/bootstrap.js" data-claw-token="${token}"></script>`
}

export function buildGtmSnippet(appUrl: string, token: string) {
  const base = appUrl.replace(/\/$/, '')
  return `<script>window.clawWidgetToken="${token}";</script>\n<script defer src="${base}/api/widget/bootstrap.js"></script>`
}
