...
export default function LeadCommandCenter() {
  ...
  async function saveLead(
    lead: Lead,
    next: { status: string; followUpDueAt?: string; outcomeLabel?: string },
  ) {
    setSaving(lead.id);
    try {
  ...
  return (
    <div className="space-y-4 rounded-xl border bg-white p-4">
      ...
      <p className="text-xs text-slate-600">
        Tracks contacted, replied, interested outcomes and follow-ups due.
      </p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {(data?.leads || []).slice(0, 80).map((lead) => (
              <LeadRow
                key={lead.id}
                lead={lead}
                onSave={saveLead}
...
function LeadRow({
  ...
  const [followUpDueAt, setFollowUpDueAt] = useState(
    (lead.follow_up_due_at || "").slice(0, 10),
  );
